...
**/
	...
	tBTNodePtr save = NULL;
	// Probihej, dokud nenajdes prvni volne misto
	while (iterator != NULL) {
		// Uloz aktualni uzel
		save = iterator;
		// Jestli je dany obsah mensi nez aktualni obsah, posun se doleva
		if (Content < iterator->Cont) {
			iterator = iterator->LPtr;
		}
		// Jestli je dany obsah vetsi nez aktualni obsah, posun se doprava
		else if (Content > iterator->Cont) {
			iterator = iterator->RPtr;
		}
		else if (Content == iterator->Cont) {
			// Jestli je obsah rovny obsahu jiz existujiciho uzlu, nedelej nic
			return;
		}
	}
	// Jestli jsme prosli sem, bunka s danou hodnotou jeste neexistuje a muzeme ji vytvorit
	tBTNodePtr new = malloc(sizeof(struct tBTNode));
	// Jestli se malloc nepovedl, vratime se 
	if (new == NULL) {
		return;
	}
	new->Cont = Content;
	...
	// Jestlize je hodnota save NULL, neni se kam posunout a vlozime tedy hodnotu jako koren
	if (save == NULL) {
		*RootPtr = new;
	}
	// Jinak mame v 'save' ulozeny posledni neprazdny uzel
	else {
		// Porovnej hodnotu Cont a na jejim zaklade vloz bud doprava nebo doleva	
		if (Content < save->Cont) {
			save->LPtr = new;
		}
		else {
			save->RPtr = new;
...
**/
	...
	// Posunuj se doleva dokud muzes
	while (ptr != NULL) {
		// Uloz aktualni ukazatel na zasobnik
		SPushP(Stack, ptr);
		// Zavolej BTWorkOut s prave zpracovavanym uzlem
		BTWorkOut(ptr);
		// Posun se doleva
		ptr = ptr->LPtr;
	}
...
**/
	...
	// Uloz uzly na zasobnik
	Leftmost_Preorder(RootPtr, &stack);
	while (!SEmptyP(&stack)) {
		// Nacti uzel z vrcholu zasobniku
		RootPtr = STopPopP(&stack);
		// Pokud ma zpracovany strom pravy podstrom, zpracuj i ten
		if (RootPtr->RPtr != NULL) {
			// Uloz na zasobnik uzly z pravych podstromu
...
**/
	...
	// Posunuj se doleva dokud muzes
	while (ptr != NULL) {
		// Uloz zpracovavany uzel na zasobnik
		SPushP(StackP, ptr);
		// Uloz, ze je uzel zpracovany, na zasobnik
		SPushB(StackB, TRUE);
		ptr = ptr->LPtr;
	}
}
...
**/
	...
	// Zkontroluj, jestli neni RootPtr prazdny
	if (RootPtr == NULL) {
		return;
	}
	...
	// Uloz uzly na zasobnik, kazdemu prirad bool TRUE
	Leftmost_Postorder(RootPtr, &stack, &stack_b);
	while (!SEmptyP(&stack)) {
		// Nacti uzel z vrcholu zasobniku
		RootPtr = STopPopP(&stack);
		// Nacti bool ze zasobniku
		left = STopPopB(&stack_b);
		// Jestlize prochazime poprve
		if (left) {
			// Dame na bool zasobnik false jako indikator, ze jsme uz uzel prosli
			SPushB(&stack_b, FALSE);
			// Vratime uzel na zasobnik
			SPushP(&stack, RootPtr);
			// Zpracujeme i jeho dalsi leve uzly
			Leftmost_Postorder(RootPtr->RPtr, &stack, &stack_b);
		}
		else {
		// Uzel prochazime podruhe, muzeme ho zpracovat
		BTWorkOut(RootPtr);
		}
	}
}
