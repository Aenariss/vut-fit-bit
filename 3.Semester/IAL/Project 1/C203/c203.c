...
*/
	...
	if (q == NULL) {			//osetreni invalid fronty
		queueError(QERR_INIT);
		return;
	}
	else {
		q->f_index = q->b_index = 0;				// nastav zacatek a prvni volny na 0

	  	for (int i = 0; i < QUEUE_SIZE; i++) {		// projdi celou frontu 
			q->arr[i] = '*';						// nastav chary v ni na '*'
	  }
	}
}
...
*/
	...
	return (q->f_index == q->b_index);		//jestlize se zacatek a prvni volny rovnaji, je prazdna
}
...
*/
	...
	return (q->f_index == nextIndex(q->b_index)); // jestli se dalsi volny prvek rovna zacatku fronty, je plna
}
...
*/
	int result = queueEmpty(q);
		if (result) {
			queueError(QERR_FRONT);
			return;
	  	}
	  	*c = q->arr[q->f_index];		//prirad do *c char ze zacatku fronty
...
*/
	...
	q->f_index = nextIndex(q->f_index);	//presun zacatek fronty na dalsi index
}
...
*/
	int result = queueEmpty(q);
	  	...
	  	queueFront(q, c);		//zapis do *c char ze zacatku
	  	queueRemove(q);			//posun index zacatku na dalsi pozici
}
...
*/
	...
	int result = queueFull(q);
	  	if (result) {
		  	queueError(QERR_UP);
		  	return;
	 	}
	  	q->arr[q->b_index] = c;					//zapis do prvniho volneho mista c
	  	q->b_index = nextIndex(q->b_index);		//posun index dalsiho volneho mista na dalsi, protoze soucasny je uz plny
}
